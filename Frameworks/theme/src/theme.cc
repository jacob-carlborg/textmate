...
theme_t::decomposed_style_t theme_t::shared_styles_t::parse_styles (plist::dictionary_t const& plist)
{
	...
	get_key_path(plist, "settings.lineBackground",  res.lineBackground);
...
styles_t const& theme_t::styles_for_scope (scope::scope_t const& scope) const
{
	...
	if(styles == _cache.end())
	{
		...
		CGColorPtr lineBackground = OakColorCreateFromThemeColor(base.lineBackground, _styles->_color_space) ?: CGColorPtr(CGColorCreate(_styles->_color_space, (CGFloat[4]){   1,   1,   1,   1 }), CGColorRelease);
		...
		styles_t res(foreground, background, lineBackground, caret, selection, font, base.underlined == bool_true, base.misspelled == bool_true);
...
theme_t::decomposed_style_t& theme_t::decomposed_style_t::operator+= (theme_t::decomposed_style_t const& rhs)
{
	...
	lineBackground = rhs.lineBackground.is_blank()    ? lineBackground : blend(lineBackground, rhs.lineBackground);
