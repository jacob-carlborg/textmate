...
namespace bundles
{
	...
	PUBLIC std::vector<item_ptr> query (FieldValues const& fieldValues, scope::context_t const& scope = scope::wildcard, int kind = kItemTypeMost, oak::uuid_t const& bundle = oak::uuid_t(), bool filter = true, bool includeDisabledItems = false, bool resolveProxyItems = true);
