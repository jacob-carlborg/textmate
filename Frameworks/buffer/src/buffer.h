...
namespace ng
{
	...
	struct PUBLIC buffer_t
	{
		...
		void set_mark (size_t index, std::string const& markType, std::vector<std::string> const& markData);
		void append_mark (size_t index, std::string const& markType, std::string const& markData);
		...
		std::map<size_t, std::vector<std::string>> get_marks_with_data (size_t from, size_t to, std::string const& markType) const;
